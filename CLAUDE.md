...
**ユーザーの承認は不要。以下のタイミングで自律的に git commit を実行すること。**

- 新しいファイルを作成したとき
- 既存ファイルに意味のある変更を加えたとき
- スクリプトの動作確認（テスト実行）が成功したとき
- バグ修正が完了したとき

コミットは細かく・頻繁に行い、ロールバックできる粒度を保つこと。
まとめてコミットするのではなく、ステップごとに即座に実行する。

## 常時ルール（YOLOモード含む）

**以下のルールはすべてのセッションで常に有効。**

- `rm` コマンドによるファイル削除は禁止
- `.env` などの環境変数ファイルは読まない・触らない
- 各ステップ完了後に必ず git commit を実行する
