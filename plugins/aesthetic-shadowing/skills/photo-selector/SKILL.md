...
> 3. **JPEGフォルダのパス**: セレクト対象の画像が入っているディレクトリ
> 4. **XMP出力先**（任意）: Stage1済みXMLがある場合のディレクトリ

ユーザーがまとめて答えてくれた場合は、確認の往復なしに即 `session.json` を生成して Step 1 へ進む。
...
確認できたら `OUTPUT_DIR` を定義し、`session.json` を生成する。

**OUTPUT_DIR の定義**: `jpeg_dir` の親ディレクトリ（例: `/path/to/photos/` が `jpeg_dir` なら `OUTPUT_DIR=/path/to/photos`）。
ユーザーが出力先を別途指定した場合はそちらを優先する。
以降のすべてのステップで `$OUTPUT_DIR` を中間ファイルの置き場として使う。
...
| `$OUTPUT_DIR/xmp_rated/*.xmp` | Lightroom対応レーティング | —（最終出力） |
