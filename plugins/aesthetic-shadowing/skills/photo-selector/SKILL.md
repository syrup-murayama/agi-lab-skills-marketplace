...
Claudeはユーザーに事前に案内する:
> 「これから代表写真が順番に表示されます。各写真に1〜5でレーティングをつけてください。
> 迷ったら直感で構いません。6分ほどで完了します。」
...
| Stage | 内容 | 状態 |
|-------|------|------|
| Stage 4 | レーティング結果から審美眼ルールをLLMが抽出 | 実装中 |
| Stage 5 | 全カットにルール適用してバッチ採点 | 実装中 |
| Stage 6 | XMPサイドカー書き出し（Lightroom連携） | 実装中 |

現時点では **Stage 3 完了後に `rated_samples.json` の結果を報告して終了**。
Stage 4-6 が完成次第、このSKILLに自動的に追加される。
