...
**失敗カットに気づかず感動する瞬間を壊さない、写真セレクト支援AI**
...
## ★ The Solution — Stage 2 ダッシュボードが主役

Aesthetic Shadowing の中心はこのダッシュボードにある。

![Stage2 ダッシュボード](docs/stage2-dashboard.png)

連写グループごとに並んだサムネイルの下に、スコアが表示される。

- **シャープネス** — ピンボケを数値化
- **露出** — 白飛び・黒潰れのリスクを可視化
- **瞳スコア** — 目を閉じているか、外れているかを検出
- **カメラ内レーティング** — 撮影時にカメラでつけた評価も引き継ぐ

**ファイルを開く前に、失敗カットがわかる。**

選ぶ前に捨てる。その体験がセレクト作業を根本から変える。
...
Stage 2: グループ化 + ダッシュボード  ←  ★ここが主役
─────────── ここから先はオプション ───────────
Stage 3: 審美眼サンプリング（代表カットを人間がレーティング）
Stage 4: 審美眼プロファイル生成（Claude が言語化）
Stage 5: CLIP バッチスコアリング（全カット自動採点・API コストゼロ）
Stage 6: Lightroom 用メタデータ書き出し（XMP サイドカー）
```
...
| ファイル | 内容 |
|----------|------|
| `stage2_report.html` | ★ 主役。グループ別サムネイル + 技術スコア |
