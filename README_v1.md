...
**誰のどんな痛みを解決するか**: イベント撮影後に2500枚の写真が積み上がるフォトグラファーの「選別地獄」。数時間の作業が、家族と過ごせたはずの時間を奪う。

**何分に短縮するか**: ユーザーの関与は **常に6分**（30枚のサンプルレーティングのみ）。残り全カットはAIが自律実行。
...
| ★4 recall（画像-画像モード） | 45% | **82%** |

※ v1.0.2（646枚・教師ラベルあり）で検証。正解=クライアント納品カット（★4）146枚
...
**インパクト (25%)**: 「家族と過ごす時間を取り戻す」という具体的なペインを解決。Lightroom連携（XMPメタデータ）で明日から実際に使える。
...
| 「運動会の写真です」と答える | 技術的失敗カット（完全白飛び・黒潰れ）を自動除外する |
| 30枚にレーティングをつける | その判断からあなたの審美眼を学習する |
| 待つ | 残り2470枚を自律的に採点し、メタデータ（JPEG直接/RAWサイドカー）を書き出す |
...
```
※ v1.0.2（646枚・教師ラベルあり）での検証。正解=クライアント納品カット（★4）146枚
